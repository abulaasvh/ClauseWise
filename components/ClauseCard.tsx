...
export const ClauseCard: React.FC<ClauseCardProps> = ({
  ...
  return (
    <div
      ...
      className={`rounded-lg border bg-white p-4 shadow-sm transition-all duration-200 border-t-2 ${borderColor} ${riskTopAccent} ${
    ...
    >
      ...
      <div className="flex items-start justify-between gap-2 mb-2.5">
        <div className="flex flex-wrap items-center gap-1.5">
          <span className="inline-flex items-center gap-1 rounded bg-slate-100 px-2 py-0.5 text-[11px] font-semibold text-slate-700 tracking-wide">
            <Tag className="h-3 w-3 text-slate-500" />
            {analysis?.category || "General Provision"}
          </span>
      ...
      {analysis?.risk_reason && (
        <div className="mb-3 rounded-md bg-slate-50 p-2.5 border border-slate-200/70 text-xs">
          <div className="font-semibold text-slate-700 mb-0.5 flex items-center gap-1">
            <ShieldCheck className="h-3.5 w-3.5 text-slate-500" />
            Risk Assessment Factor
          </div>
          <p className="text-slate-600 leading-normal">{analysis.risk_reason}</p>
        </div>
      )}
        ...
        <div className="mb-3">
          ...
          <div className="flex flex-wrap gap-1.5">
            {analysis.key_terms.map((term, idx) => (
              <span
                key={idx}
                className="rounded bg-slate-100 px-2 py-0.5 text-[11px] font-mono font-medium text-slate-700 border border-slate-200"
              >
                {term}
              </span>
            ))}
      ...
      {notes.length > 0 && (
        <div className="mb-3 space-y-1.5 pt-2 border-t border-slate-100">
          <div className="text-[10px] font-semibold uppercase tracking-wider text-amber-800 flex items-center gap-1">
          ...
          {notes.map((note) => (
            <div
              key={note.id}
              className="rounded bg-amber-50/70 border border-amber-200/80 p-2 text-xs text-amber-950"
            >
              {note.noteText}
            </div>
          ))}
      ...
      {showNoteInput && (
        <div className="mb-3 pt-2 border-t border-slate-100">
          <textarea
            ...
            className="w-full rounded border border-slate-200 p-2 text-xs text-slate-800 focus:outline-none focus:ring-1 focus:ring-slate-400"
          />
          <div className="flex items-center justify-end gap-1.5 mt-1.5">
            ...
            <button
              ...
              className="rounded bg-slate-900 px-2.5 py-0.5 text-[11px] font-semibold text-white hover:bg-slate-800 disabled:opacity-50"
            >
      ...
      <div className="flex items-center justify-between pt-2.5 border-t border-slate-100 text-xs text-slate-500">
        <div className="flex items-center gap-3">
          <button
            onClick={onSelect}
            type="button"
            className="inline-flex items-center gap-1 font-medium text-slate-700 hover:text-slate-900 transition hover:underline"
          >
        ...
        {onReanalyze && (
          <button
            ...
            type="button"
            className="inline-flex items-center gap-1 text-[11px] text-slate-500 hover:text-slate-800 transition disabled:opacity-50"
